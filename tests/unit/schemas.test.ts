...
describe("AmountSchema", () => {
	test("should accept valid SLE amount", () => {
		expect(() => validate(AmountSchema, { currency: "SLE", value: 1000 })).not.toThrow();
	});

	test("should accept valid USD amount", () => {
		expect(() => validate(AmountSchema, { currency: "USD", value: 500 })).not.toThrow();
	});

	test("should accept zero value", () => {
		expect(() => validate(AmountSchema, { currency: "SLE", value: 0 })).not.toThrow();
	});

	test("should reject negative value", () => {
		expect(() => validate(AmountSchema, { currency: "SLE", value: -100 })).toThrow(
			MonimeValidationError,
		);
	});

	test("should reject invalid currency", () => {
		expect(() => validate(AmountSchema, { currency: "EUR", value: 100 })).toThrow(
			MonimeValidationError,
		);
	});

	test("should reject missing currency", () => {
		expect(() => validate(AmountSchema, { value: 100 })).toThrow(MonimeValidationError);
	});

	test("should reject missing value", () => {
		expect(() => validate(AmountSchema, { currency: "SLE" })).toThrow(MonimeValidationError);
	});
});
...
describe("MetadataSchema", () => {
	...
	test("should reject more than 64 keys", () => {
		...
		expect(() => validate(MetadataSchema, metadata)).toThrow(MonimeValidationError);
	});
...
describe("CreatePayoutInputSchema", () => {
	...
	test("should reject missing destination", () => {
		...
		expect(() => validate(CreatePayoutInputSchema, input)).toThrow(MonimeValidationError);
	});
});
...
describe("CreateCheckoutSessionInputSchema", () => {
	test("should accept valid checkout session", () => {
		...
		expect(() => validate(CreateCheckoutSessionInputSchema, input)).not.toThrow();
	});
...
describe("CreateWebhookInputSchema", () => {
	...
	test("should reject empty events array", () => {
		...
		expect(() => validate(CreateWebhookInputSchema, input)).toThrow(MonimeValidationError);
	});
